...
/**
 新增方法，用于高级设置--消息类型的按钮选择，图片在左，文本在右，整体剧中,currentImage判断按钮有图片和无图片的case显示
 */
#pragma mark - 居中对齐,图片在左
- (void)alignmentCenter_ImgLeft{
    if (self.currentImage) {
        
        //    设置文本的坐标
        CGFloat imgX = (ml_btnWidth - ml_labelWidth -ml_imageWidth - ml_padding) * 0.5;
        CGFloat imgY = (ml_btnHeight - ml_imageHeight) * 0.5;
        
        //    设置img的frame
        self.imageView.frame = CGRectMake(imgX, imgY, ml_imageWidth, ml_imageHeight);
        
        //    设置label的坐标
        CGFloat labelX = CGRectGetMaxX(self.imageView.frame) + ml_padding;
        CGFloat labelY = (ml_btnHeight - ml_labelHeight) * 0.5;
        //    设置图片的frame
        self.titleLabel.frame = CGRectMake(labelX, labelY, ml_labelWidth, ml_labelHeight);
    }else{
        //    设置文本的坐标
        CGFloat labelX = (ml_btnWidth - ml_labelWidth) * 0.5;
        CGFloat labelY = (ml_btnHeight - ml_labelHeight) * 0.5;
        //    设置label的frame
        self.titleLabel.frame = CGRectMake(labelX, labelY, ml_labelWidth, ml_labelHeight);
    }
...
//    }
    ...
    }else if (_status ==MLAlignmentStatusCenter_ImgLeft){
        [self alignmentCenter_ImgLeft];
    }
