...
@interface DYTitleClickScrollView ()<UIScrollViewDelegate>
//UI
...
//初始的类型
@property (nonatomic, assign) EScrollType originalType;
//选中的index
...
@implementation DYTitleClickScrollView
...
- (instancetype)init {
    self = [super init];
    if (self) {
        self.originalType = eScrollTypeFullScreen;
        [self initSubView];
    }
    return self;
}
...
- (void)initSubView {
    ...
    self.bottomView.backgroundColor = DYAppearanceColorFromHex(0xCEA76E, 1.0);
    [self.scrollView addSubview:self.bottomView];
    self.underLine = [[UIView alloc]init];
    self.underLine.backgroundColor = DYAppearanceColor(@"H2", 1.0);
    [self addSubview:self.underLine];
    self.index = 0;
    self.contentWidth = DYScreenWidth;
    //样式
...
- (void)layoutSubviews {
    ...
    self.underLine.frame = CGRectMake(0, rect.size.height - 0.5,rect.size.width, 0.5);
}
...
- (void)setTitleArray:(NSArray <NSString *>*)titlesArray selectedIndex:(NSInteger)index {
    ...
    if ([self changedTitles:titlesArray]) {
        ...
        if (titlesArray.count >0 && totalWidth+Margin*2 < self.contentWidth && self.originalType == eScrollTypeFullScreen) {
